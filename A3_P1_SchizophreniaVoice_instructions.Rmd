...
read_pitch <- function(filename) {
    ...
    return(data_frame(Study, Participant, Trial, Diagnosis, Mean, IQR, SD))
}
...
art$Study <-
    art$soundname %>% 
    str_extract("Study\\d") %>% gsub("Study", "", .)

# Finally the pitch data
pitch <- read_csv("pitch_data.csv")

# Now we merge them
df_1 <- merge(demo, pitch, by = c("Participant", "Study"), all = T)
df <- merge(df_1, art, by = c("Participant", "Study"), all = T)
